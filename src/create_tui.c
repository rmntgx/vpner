...
static void print_toggle_line(int selected, int index, const char *label, bool is_enabled) {
...
static void print_action_item(int selected, int index, const char *label, const char *color) {
...
static void print_editable_field(int selected, int index, const char *label,
...
static void print_create_menu(ConfigOptions *opt, const char *urlbuf, int url_len,
    ...
    print_editable_field(selected, 0, "URL", urlbuf, url_len, URL_FIELD_WIDTH, url_valid);

    print_toggle_line(selected, 1, "TUN", opt->tun);

    print_toggle_line(selected, 2, "Proxy", opt->proxy);

    print_editable_field(selected, 3, "Proxy port", portbuf, port_len, PORT_FIELD_WIDTH, port_valid);

    // print_toggle_line(selected, 4, "Fake IP", opt->fakeip);
    ...
    print_action_item(selected, 6, "Continue", "\033[92m"); // green
    print_action_item(selected, 7, "Cancel", "\033[91m"); // red
}

/* Очистка блока меню (menu_lines строк) — полезно перед открытием подменю */
static void clear_menu_block(int menu_lines) {
    printf("\033[u"); /* restore saved pos */
	for (int i = 0; i < menu_lines; i++) printf("\033[K\n");
    printf("\033[u"); /* restore saved pos */
    fflush(stdout);
...
/**
   ...
   -1 - cancel
*/
static int handle_create_input(int count, int *selected,
...
ConfigOptions* create_cfgopt(char **out_url) {
    ...
    print_create_menu(opt, urlbuf, url_len, portbuf, port_len, selected);
    fflush(stdout);

    int result = 0;
    while (1) {
    int r = handle_create_input(items, &selected,
